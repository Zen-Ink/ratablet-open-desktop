...
将 reMarkable 设备用作 Linux、Windows 或 macOS 数位板。

ratablet 默认以系统托盘应用运行。点击托盘图标可打开控制面板。面板显示设备型号、连接状态、旋转方向、笔映射和设置。界面语言跟随系统，也可手动选择简体中文或英文。

设置页接受设备 IP 地址或 SSH 目标。初始目标为 `root@10.11.99.1`。身份验证优先使用 SSH 密钥。保存的密码进入 Linux Secret Service、Windows Credential Manager 或 macOS Keychain。身份验证失败会暂停重连并打开密码对话框。

SSH 仅负责验证设备并启动远端读取进程。笔事件通过带一次性会话令牌的临时 TCP 连接返回。连接期间平板会自动保持唤醒。连接断开后程序持续重连，按 Ctrl+C 可结束进程。事件流本身不加密，还请使用 USB 或可信网络。

## 性能

RM2 USB 实测传输间隔平均为 1.85 毫秒，没有出现 10 毫秒或更长的停顿；不过最终体验还取决于宿主机和绘画软件。
...
输入节点按名称自动发现。显式校准值可支持更多型号。RM1 和 RM2 使用横屏坐标。Paper Pro 系列采用 90 度旋转。`--rotate` 可选择固定方向，`--auto-rotate` 可跟随 Paper Pro 旋转传感器。
